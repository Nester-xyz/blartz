...
const CardsForExplore = ({ item }: Props) => {
    ...
    const getNFTDetails = async () => {
        try {
            collectionContract = new web3.eth.Contract(BlastNFTABI.abi, item.collection);
            const tokenURIandName = await collectionContract.methods.getURIAndName(BigInt(item.tokenId)).call({ from: walletAddress })
            console.log(tokenURIandName);
            setTitle(tokenURIandName[1]);
            setImgSrc(`https://red-mad-woodpecker-146.mypinata.cloud/ipfs/${tokenURIandName[0]}`)
        } catch (error) {
    ...
    return (
        <div className="w-8/12 mx-auto">
            <div className="w-full aspect-square border cursor-pointer" onClick={() => { window.location.href = `/nft/${item.collection}/${item.tokenId}`; }}>
                <Image
                    src={imgSrc}
                    alt="profile"
                    sizes="100%"
